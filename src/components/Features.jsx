    ...
    async function fetchCourses() {
      try {
        ...
        if (!res.ok) {
          ...
          console.error("Fetch /api/courses failed:", res.status, res.statusText, text);
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {featuredCourses.map((course) => (
            <div
              key={course._id}
              className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 flex flex-col h-[520px]"
            >
              {/* Course Image */}
